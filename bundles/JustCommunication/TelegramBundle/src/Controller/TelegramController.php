...
use App\Service\RedisHelper;
use App\Service\SmsAeroHelper;
use JustCommunication\TelegramBundle\Service\TelegramHelper;
use JustCommunication\TelegramBundle\Service\Telegram\TelegramWebhook;
...
class TelegramController extends AbstractController
{
    ...
    public function telegram_webhook(TelegramHelper $telegram, Request $request, ParameterBagInterface $services_params, SmsAeroHelper $smsAeroHelper, UserRepository $userRepository){

        $webhook = new TelegramWebhook();
        $webhook->setTelegramHelper($telegram);
        $webhook->setServicesYamlParams($services_params);
        $webhook->setSmsAeroHelper($smsAeroHelper);
        $webhook->setUserRepository($userRepository);
        //$x = $params->get('smsaero');
        //dd($x);
        ...
        if ($token == $telegram->config['token']) {
            ...
            try {
                ...
                // mess это текст, а message это уже объектб а не уй собачий
                        ...
                        if (method_exists(TelegramWebhook::class, $command.$user['role'].'Command')){
                            ...
                            if (is_array($row)){
                                if ($row['active'] == 0){
                                    ...
                                    $telegram->updateUserSubscribe($user_chat_id, $_subscription, 1);
                                    $ans = '"' . $list[$_subscription]['note'] . '" успешно оформлена, снова.';
                                }else{
                                    $ans = '"' . $list[$_subscription]['note'] . '" уже подключена.';
                                }
                            }else{
                                $telegram->addUserSubscribe($user_chat_id, $_subscription);
                            ...
                            if (is_array($row) && $row['active'] == 1){
                                ...
                                $telegram->updateUserSubscribe($user_chat_id, $_subscription, 0);
